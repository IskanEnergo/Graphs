...
fn main() {
    ...
    let (source, sink, max_flow, residual_graph) = graph_tool::algorithms::ford_fulkerson(graph.deref());

    for line in &residual_graph {
        println!("{:?}", line);
    }

    println!("{} - maximum flow from {} to {}", max_flow, source, sink);


    for i in 0..graph.count_vertex() {
        for j in 0..graph.count_vertex() {
            if graph.is_edge(i, j) {
                println!("{} {} {}/{}", i + 1, j + 1, residual_graph[j][i], graph.weight(i, j))
            }
        }
    }
}